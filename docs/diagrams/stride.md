...
| RCS_P4 (F14) | T | Staff hand-declare a cure / close a case that isn't resolved | Both closed states are reachable ONLY through the arrears close pass reading the loan's persisted facts (cure = left NPL or loan closed; written-off closes as `closed_written_off`) via the single `domain/recovery.py:transition` gatekeeper; idempotent by side-effect counts | Richer staff dispositions + the single outcome note are **INCOMING (!53 / issue #23, 0033)** — that MR adds the corresponding rows here |
...
obligations above; **!53 / issue #23 (0033)** inherits the RCS_P4 row
(dispositions + outcome note). P23 consumes this file as its DAST
triage map.
