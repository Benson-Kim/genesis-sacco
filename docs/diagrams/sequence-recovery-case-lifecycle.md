<!--
  ...
  INCOMING (!53 / issue #23, migration 0033): pause dispositions
  (disputed, irrecoverable-pending-write-off), the staff-attested
  closed_restructured terminal, and the single post-closure outcome
  note are drawn DASHED below and flipped as-built by that MR
  (v1.2 rule 11).
...
non-performing to open one), and each loan has at most one open case.
...
closes cases) or when the committee writes the loan off. No staff
member can hand-close a case on main today.
...
sequenceDiagram
    ...
    participant CASE as Case file<br/>(one open per loan)
    ...
    alt loan is performing (or not active)
        ...
        SYS->>CASE: claim the one-open-case slot
    ...
    JOB->>CASE: scan open cases (safe to re-run)
    ...
    Note over O,CASE: INCOMING (!53 / issue #23 — NOT on main yet)
    O-->>SYS: pause dispositions: disputed /<br/>irrecoverable-pending-write-off
    O-->>SYS: staff-attested closed-restructured terminal
    O-->>SYS: exactly one closing outcome note at/after closure
    end
```
...
| One-open-case slot | atomic claim on `uq_recovery_cases_one_open` (0026, `WHERE status = 'open'`) via `ON CONFLICT DO NOTHING` + rowcount; 0026 CHECKs (NPL set, dpd > 90) are the DB backstop |
...
| Job-only close | `application/arrears.py:run_arrears_for_tenant` → `application/recovery.py:run_recovery_close_pass` (runs AFTER the classify pass, so today's cure closes today) → `_close_one` — every close passes `domain/recovery.py:transition` (the single gatekeeper; OPEN → CLOSED_CURED / CLOSED_WRITTEN_OFF, both terminal, self-transitions illegal); worker-actor audit rows (`actor_id=None`) |
| Safe re-run | close scan matches `status = 'open'` only, case rows `FOR UPDATE SKIP LOCKED` in id order, joined loan read WITHOUT a lock (MVCC — the job's own persisted classification); idempotent by side-effect counts |
| "new case, never reopened" | `domain/recovery.py` — both closed states have empty transition sets (addendum A6) |
...
| INCOMING block | !53 / issue #23 (migration 0033): `disputed`, `irrecoverable_pending_write_off`, `closed_restructured`, `recovery_case_notes.is_outcome` — drawn dashed; that MR flips this diagram, dfd.md F14 and the stride.md RCS_P4 row in the same MR (rule 11) |
