<!--
  ...
  workflow columns + register index on share_transfers; 0039 is
  claimed by the in-flight batch-8 MR !79 and is absent from this
  tree — 0040 is authored on down_revision 0038 and re-chains onto
  0039 when !79 merges, declared in !83) ships in that MR
...
flowchart TB
    ...
    PG[("PostgreSQL 16 — FORCED RLS on every tenant table<br/>ADR-0002; genesis/infrastructure/tenancy.py<br/>alembic head 0040")]
...
| PostgreSQL 16, forced RLS | as-built | RLS enabled AND forced per ADR-0002 (`docs/adr/`), session scoping `genesis/infrastructure/tenancy.py` (`tenant_session` L12); migration head `0040` (`backend/migrations/versions/0040_share_transfer_maker_checker.py`) |
