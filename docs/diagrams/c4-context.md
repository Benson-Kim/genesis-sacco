...
| PostgreSQL 16, forced RLS | as-built | RLS enabled AND forced per ADR-0002 (`docs/adr/`), session scoping `genesis/infrastructure/tenancy.py` (`tenant_session` L12); migration head `0043` (`backend/migrations/versions/0043_external_txn_ref_and_search_index.py`; `down_revision = "0041"` on this tree — `0042` is the in-flight !87 claim, re-chained onto it when !87 merges) |
