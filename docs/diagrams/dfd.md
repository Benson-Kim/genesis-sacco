...
money talks and closes cases. (Richer pause dispositions — disputed,
irrecoverable-pending-write-off — and a single closing outcome note
are **INCOMING (!53 / issue #23)**, not on main yet.)
...
flowchart LR
    ...
    subgraph TB1["TB1 — signed-in staff only"]
        RCS_P1(["open a case: only a<br/>non-performing loan,<br/>one open case per loan"])
        RCS_P2(["assign a colleague<br/>(auditors excluded);<br/>add permanent notes"])
...
| RCS_P4 | `application/arrears.py:run_arrears_for_tenant` → `application/recovery.py:run_recovery_close_pass` (after the classify pass, so a loan curing today closes its case today) → `_close_one` via `domain/recovery.py:transition` (the single gatekeeper; both closed states terminal); cure = stored classification left `NPL_CLASSES` or loan `closed`; `written_off` closes as `closed_written_off`; idempotent by side-effect counts | case rows `FOR UPDATE SKIP LOCKED` in id order, joined loan read WITHOUT a lock (§3 recovery close-pass row) |
| RCS_S1–S6 | `recovery_cases` + `recovery_case_notes` (0026), `loans` (0001/0026 idx), `users`/`roles`/`permissions` (0001), `audit_log`, `outbox_events`; the !53 dispositions/outcome-note columns are **INCOMING (0033)** — that MR updates this diagram and stride.md in the same MR (rule 11) | — |
