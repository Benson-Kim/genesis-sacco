<!--
  ...
  Migration head 0038 -> 0040 by the issue-#31 batch-10 MR (!83):
  ...
  status-machine trigger, register expression index; 0039 is claimed
  by the in-flight batch-8 MR !79 and absent from this tree — 0040
  re-chains onto 0039 when !79 merges, declared in !83) ships in
  that MR (same-commit refresh per v1.2 rule 11 / spot-check check 5).
...
flowchart TB
    ...
    MIG["Migration runner — alembic upgrade head<br/>backend/alembic.ini + backend/migrations/<br/>versions 0001..0038 + 0040 (head 0040 on this tree; 0039 in flight on !79)"]
...
| migration runner | `backend/alembic.ini`, `backend/migrations/env.py`, `backend/migrations/versions/0001..0038` + `0040` — head `0040` (`0040_share_transfer_maker_checker.py`, `down_revision = "0038"`, shipped by the issue-#31 batch-10 MR !83 in the same commits as this refresh), verified against `versions/` on this tree (nothing claims `0040` as parent; `0039` is claimed by the in-flight !79 and 0040 re-chains onto it at merge, declared in !83) |
