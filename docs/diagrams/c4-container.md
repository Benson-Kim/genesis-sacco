<!--
  ...
  Migration head 0038 -> 0039 by the issue-#31 batch-8 MR:
  0039_member_dividend_payout.py (expand-only: members gains one
  nullable CHECK-pinned payout-preference column) ships in that MR
  (same-commit refresh per v1.2 rule 11 / spot-check check 5).
  Migration head 0039 -> 0040 by the issue-#31 batch-10 MR (!83):
  ...
  the batch-8 merge landed on main, as declared up front in !83) ships
  in that MR (same-commit refresh per v1.2 rule 11 / spot-check check 5).
...
flowchart TB
    ...
    MIG["Migration runner — alembic upgrade head<br/>backend/alembic.ini + backend/migrations/<br/>versions 0001..0040 (head 0040 on this tree; 0040 re-chained onto 0039 after the batch-8 merge)"]
...
| migration runner | `backend/alembic.ini`, `backend/migrations/env.py`, `backend/migrations/versions/0001..0040` — head `0040` (`0040_share_transfer_maker_checker.py`, `down_revision = "0039"`, shipped by the issue-#31 batch-10 MR !83; re-chained onto `0039_member_dividend_payout.py` after the batch-8 merge landed on `main`, as declared up front in !83), verified against `versions/` on this tree (nothing claims `0040` as parent) |
