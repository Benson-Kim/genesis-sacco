...
flowchart TB
    ...
    MIG["Migration runner — alembic upgrade head<br/>backend/alembic.ini + backend/migrations/<br/>versions 0001..0040 (head 0040; 0039 merged via !79)"]
...
| migration runner | `backend/alembic.ini`, `backend/migrations/env.py`, `backend/migrations/versions/0001..0040` — head `0040` (`0040_members_numeric_member_no_index.py`, `down_revision = "0039"` — re-chained from `0038` after !79 merged `0039_member_dividend_payout.py`, the 0017 precedent; shipped by the issue-#31 batch-7 remediation MR in the same commit as this refresh), verified against `versions/` on this tree (nothing claims `0040` as parent) |
