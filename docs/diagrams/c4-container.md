<!--
  ...
  refresh per v1.2 rule 11 / spot-check check 5).
  Migration head 0041 -> 0043 by the issue-#35 remainder MR:
  ...
  text_pattern_ops prefix index; no table/RLS change; authored with
  down_revision '0041' — 0042 is the in-flight !87 claim, never
  renumbered here; this revision re-chains onto 0042 when !87 merges)
...
flowchart TB
    ...
    MIG["Migration runner — alembic upgrade head<br/>backend/alembic.ini + backend/migrations/<br/>versions 0001..0041 + 0043 (head 0043 on this tree; 0042 is the in-flight !87 claim; 0043 shipped by the issue-#35 remainder MR)"]
...
| migration runner | `backend/alembic.ini`, `backend/migrations/env.py`, `backend/migrations/versions/0001..0041` + `0043` — head `0043` (`0043_external_txn_ref_and_search_index.py`, `down_revision = "0041"` on this tree — `0042` is the in-flight !87 claim, never renumbered here; this revision re-chains onto `0042` in its own MR when !87 merges, the 0017/0041 precedent; shipped by the issue-#35 remainder MR in the same commit as this refresh), verified against `versions/` on this tree |
