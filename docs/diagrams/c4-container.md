...
flowchart TB
    ...
    MIG["Migration runner — alembic upgrade head<br/>backend/alembic.ini + backend/migrations/<br/>versions 0001..0040 (head 0040; 0039 = !79's in-flight rule-14 claim)"]
...
| migration runner | `backend/alembic.ini`, `backend/migrations/env.py`, `backend/migrations/versions/0001..0040` — head `0040` (`0040_members_numeric_member_no_index.py`, `down_revision = "0038"`, shipped by the issue-#31 batch-7 remediation MR in the same commits as this refresh; 0039 is !79's in-flight rule-14 claim, re-chained under 0040 when it lands), verified against `versions/` on this tree (nothing claims `0040` as parent) |
