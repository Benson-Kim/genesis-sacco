...
flowchart TB
    ...
    MIG["Migration runner — alembic upgrade head<br/>backend/alembic.ini + backend/migrations/<br/>versions 0001..0039 (head 0039 verified at reconciliation)"]
...
| migration runner | `backend/alembic.ini`, `backend/migrations/env.py`, `backend/migrations/versions/0001..0039` — head `0039` (`0039_member_dividend_payout.py`, `down_revision = "0038"`, shipped by the issue-#31 batch-8 MR in the same MR as this refresh), verified against `versions/` on this tree (nothing claims `0039` as parent) |
