...
def test_concurrent_application_create_waits_for_exit_member_lock() -> None:
    """Application creation must take a member FOR SHARE lock. A concurrent
    settlement holds FOR UPDATE and marks the member exited before commit;
    create_application must wait, then reject the terminal member instead
    of inserting an open application beside an exited member."""
    ...
    async def run() -> None:
        ...
        assert created is False, "application create must wait and then reject exited member"
        async with tenant_session(factory(), tid) as session:
            apps = (
                await session.execute(
                    text(
                        "SELECT count(*) FROM loan_applications WHERE member_id = CAST(:m AS uuid)"
                    ),
                    {"m": str(mid)},
                )
            ).scalar_one()
