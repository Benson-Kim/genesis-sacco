...
async def _fetch_raw(
    ...
    columns = ", ".join(_SETTING_KEYS)  # code-owned registry keys (v1.1 rule 6)
    row = (
    ...
    row = (
        await session.execute(
            text(
                ...
                "SELECT committee_quorum FROM tenant_settings "
                "WHERE tenant_id = CAST(:tid AS uuid)"
            ),
            {"tid": str(tenant_id)},
        )
...
async def approval_bands_config(
    ...
    row = (
        await session.execute(
            text(
                # Explicit tenant predicate on top of RLS (v1.1 rule 4).
                "SELECT approval_bands FROM tenant_settings "
                "WHERE tenant_id = CAST(:tid AS uuid)"
            ),
            {"tid": str(tenant_id)},
        )
    ...
    if row is None or row[0] is None:
        return None
    return cast(
        "tuple[ApprovalBand, ...]", _revalidated_bands("approval_bands", row[0])
    )
