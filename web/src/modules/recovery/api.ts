/**
 * Recovery API layer (P15 follow-on batch 1, issue #31 — the P13.16
 * collections & recovery-case API) over the GENERATED client.
 *
 ...
 *   body carries the target-paired rationale fields only (!53 F1/F2);
 *   keys the target does not require are NOT sent at all.
 * - There is NO cure/write-off close route and NO note edit/delete
 *   route on the contract (append-only, addendum A2) — this layer
 ...
 * assurance roles (audit independence — all server-vetted, addendum
 * A4; this client sends the id and lets the server judge). The version
...
/**
 * Record a staff disposition (loan_book:edit; issue #23 N2, !53
 * F1/F2): pause (reason REQUIRED), resume, or close as restructured
 ...
 * A2: append-only — no edit or delete route exists). */
...
/** Record THE post-closure outcome note (loan_book:edit; issue #23
 * N3): exactly one per case, at/after closure only — a second attempt
